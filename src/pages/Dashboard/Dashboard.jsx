...
export default function Dashboard() {
  ...
  async function loadStats() {
    ...
    try {
      ...
      // أحدث 5 أسر
      const sorted = [...families].sort((a,b)=>new Date(b.updated_at||0)-new Date(a.updated_at||0)).slice(0,5)
      const campMap = Object.fromEntries(camps.map(c=>[c.id,c.name]))
      setRecent(sorted.map(f=>({...f, campName: campMap[f.camp_id]||'—'})))
      // بطاقة المخيم للمندوب
      if (isCampDelegate && profile?.camp_id) {
        ...
        if (camp) {
          ...
          setCampInfo({ ...camp, familiesCount: campFams.length, membersCount: campFams.length + members.filter(m=>campFams.some(f=>f.id===m.family_id)).length })
        }
      }
      // إحصائيات المزامنة
      ...
      if (navigator.onLine) {
        ...
        setStats(s=>({...s, families:fRes.count??s.families, camps:cRes.count??s.camps}))
      }
  ...
  const greet = () => { const h=new Date().getHours(); return h<12?'صباح الخير':h<17?'مساء الخير':'مساء النور' }
    ...
    <div>
      ...
      {campInfo && (
        <div className="bg-gradient-to-r from-accent/10 to-transparent border border-accent/30 rounded-2xl p-4 mb-4">
          ...
          <div className="flex gap-4 text-sm">
            <div className="text-center"><div className="font-black text-white text-lg">{campInfo.familiesCount}</div><div className="text-muted text-[10px]">أسرة</div></div>
            <div className="text-center"><div className="font-black text-white text-lg">{campInfo.membersCount}</div><div className="text-muted text-[10px]">فرد</div></div>
            {campInfo.capacity>0 && <div className="text-center"><div className={`font-black text-lg ${campInfo.familiesCount/campInfo.capacity>0.9?'text-red':'text-green'}`}>{Math.round(campInfo.familiesCount/campInfo.capacity*100)}%</div><div className="text-muted text-[10px]">إشغال</div></div>}
          </div>
        </div>
      )}
      ...
      <Card title="المزامنة" icon="🔄">
        <div className="flex items-center justify-between mb-3">
          ...
          {online&&<button onClick={handleSync} disabled={syncing}
        ...
        <div className="grid grid-cols-3 gap-2">
          {[['⏳ انتظار',sync.pending,sync.pending>0?'accent':'muted'],['❌ فشل',sync.failed,sync.failed>0?'red':'muted'],['⚠️ تعارض',sync.conflicts,sync.conflicts>0?'accent':'muted']].map(([l,v,c])=>(
